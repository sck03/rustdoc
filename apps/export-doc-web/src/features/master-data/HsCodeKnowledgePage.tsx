  ...
  return <div className="knowledge-task-card knowledge-search-card">
    ...
    {search.data ? <>
      ...
      <div className="knowledge-result-list">{search.data.items.map(item => <article key={`${item.currentCode}-${item.rawCode}`} className={`knowledge-result ${item.canUse ? "usable" : "warning"}`}>
        ...
        <div className="knowledge-score"><b>{item.score}</b><span>匹配分</span><small>{item.exampleCount} 条实例</small>{canOperate ? <><button type="button" disabled={!item.canUse || feedback.isPending} onClick={() => feedback.mutate({ code: item.currentCode, accepted: true, name: item.name, specification: item.specification })}>确认适用</button><button className="text-button" type="button" disabled={feedback.isPending} onClick={() => feedback.mutate({ code: item.currentCode || item.rawCode, accepted: false, name: item.name, specification: item.specification })}>不适用</button></> : null}</div>
      </article>)}</div>
...
function ExampleLibrary({ client, canOperate, canManage }: { client: ExportDocManagerApiClient; canOperate: boolean; canManage: boolean }) {
  ...
  return <div className="knowledge-task-card"><div className="section-heading"><div><h2>申报实例库</h2><p>只保存已经人工确认、可以参与智能查询的公司经验。</p></div>{canOperate ? <button className="command-button" type="button" onClick={() => setEditing(blank)}>新增实例</button> : null}</div>
    ...
    {message ? <InlineNotice tone={message.includes("失败") || message.includes("必须") ? "error" : "success"}>{message}</InlineNotice> : null}
...
function HistoryLearning({ client, canOperate }: { client: ExportDocManagerApiClient; canOperate:boolean }) {
  ...
  const confirm = useMutation({
    mutationFn: (item: NonNullable<typeof candidates.data>[number]) => client.saveHsCodeKnowledgeExample({ body: { id: 0, rawReportedHsCode: item.rawCode, resolvedCurrentHsCode: item.currentCode, productName: item.productName, specification: item.specification, source: `HistoryConfirmed:${item.source}`, sourceYear: new Date().getFullYear(), resolutionStatus: "ManuallyVerified", isManuallyVerified: true } }),
    onSuccess: async () => { setMessage("历史经验已确认并加入申报实例库。"); await queryClient.invalidateQueries({ queryKey: ["hs-history-candidates"] }); await queryClient.invalidateQueries({ queryKey: ["hs-knowledge", "examples"] }); },
    onError: error => setMessage(readApiError(error)),
  });
  return <div className="knowledge-task-card">
    ...
    {candidates.data?.length === 0 ? <PageState title="没有新的待确认资料" description="已确认或重复的历史经验不会再次进入候选列表。" /> : null}
  </div>;
}
