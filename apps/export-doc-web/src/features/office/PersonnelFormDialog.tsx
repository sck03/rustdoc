...
export function PersonnelFormDialog({ client, user, departments, record, onClose, onSaved }: {
  ...
  return <OfficeDialog title={record ? `维护档案 · ${record.employee.fullName}` : "入职登记"} onClose={onClose} {...operation} protectChanges>
    <form className="personnel-form" onSubmit={submit} autoComplete="off">
      <fieldset className="office-form-grid" disabled={operation.busy}>
        <legend>基本信息</legend>
        {!record && <OfficeField label="工号（公司内唯一）"><input name="employeeNumber" required maxLength={40} autoFocus /></OfficeField>}
        ...
        {!record && <>
          ...
          <label className="checkbox-field"><input name="onProbation" type="checkbox" defaultChecked />入职时处于试用期</label>
        </>}
        {record && <p className="office-field-wide office-muted">{record.employee.employeeNumber} · {record.employee.departmentName} · {record.employee.jobTitle}。部门与岗位通过调岗流程变更。</p>}
