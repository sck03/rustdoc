    ...
    public class DatabaseInitializationService : IDatabaseInitializationService
    {
            ...
            string bootstrapToken)
        {
            ...
            try
            {
                ...
                await EnsureInvoiceItemPricingSchemaAsync(context, usesPostgreSql).ConfigureAwait(false);
                await EnsureInvoiceStatusHistorySchemaAsync(context, usesPostgreSql).ConfigureAwait(false);
        ...
        private const string InvoiceCompanyScopeNoTypeIndexName = "IX_Invoices_CompanyScope_InvoiceNo_Type";

        private static async Task EnsureInvoiceItemPricingSchemaAsync(
            ...
            bool usesPostgreSql)
        {
            ...
            if (usesPostgreSql)
            {
                await context.Database.ExecuteSqlRawAsync(
                    """
                    ALTER TABLE "Items"
                        ADD COLUMN IF NOT EXISTS "PriceCalculationMode" character varying(30) NOT NULL DEFAULT 'UnitPriceDriven';
                    ALTER TABLE "Items"
                        ALTER COLUMN "UnitPrice" TYPE numeric(18, 5)
                        USING ROUND("UnitPrice"::numeric, 5);
            ...
            await AddSqliteColumnIfMissingAsync(
                    ...
                    "TEXT NOT NULL DEFAULT 'UnitPriceDriven'")
                .ConfigureAwait(false);
        }

        private static async Task EnsureInvoiceStatusHistorySchemaAsync(
            AppDbContext context,
            bool usesPostgreSql)
        {
            if (!context.Database.IsRelational()) return;
            ...
            if (usesPostgreSql)
            {
                await context.Database.ExecuteSqlRawAsync(
                    """
                    CREATE TABLE IF NOT EXISTS "InvoiceStatusHistories" (
                        ...
                        "ChangedByUsername" character varying(50) NOT NULL DEFAULT '',
                        "ChangedAt" timestamp with time zone NOT NULL,
                        CONSTRAINT "FK_InvoiceStatusHistories_Invoices_InvoiceId"
                            FOREIGN KEY ("InvoiceId") REFERENCES "Invoices" ("Id") ON DELETE CASCADE
                    );
                    CREATE INDEX IF NOT EXISTS "IX_InvoiceStatusHistories_InvoiceId_ChangedAt"
                        ON "InvoiceStatusHistories" ("InvoiceId", "ChangedAt");
                    """).ConfigureAwait(false);
                return;
            }
            ...
            await context.Database.ExecuteSqlRawAsync(
                """
                CREATE TABLE IF NOT EXISTS "InvoiceStatusHistories" (
                    "Id" INTEGER NOT NULL CONSTRAINT "PK_InvoiceStatusHistories" PRIMARY KEY AUTOINCREMENT,
                    "InvoiceId" INTEGER NOT NULL,
                    "FromStatus" TEXT NOT NULL,
                    "ToStatus" TEXT NOT NULL,
                    "Note" TEXT NOT NULL DEFAULT '',
                    "ChangedByUserId" INTEGER NULL,
                    "ChangedByUsername" TEXT NOT NULL DEFAULT '',
        ...
        private static async Task EnsureSqliteInvoiceTypeSchemaAsync(AppDbContext context)
        {
            ...
            await context.Database.ExecuteSqlRawAsync(
                $"CREATE UNIQUE INDEX IF NOT EXISTS \"{InvoiceCompanyScopeNoTypeIndexName}\" ON \"Invoices\" (\"CompanyScope\", \"InvoiceNo\", \"Type\")").ConfigureAwait(false);
        }
            ...
            try
            {
                ...
                await using (var command = connection.CreateCommand())
                {
                    ...
                    while (await reader.ReadAsync().ConfigureAwait(false))
                    {
                        ...
                        if (!isUnique ||
                            string.Equals(name, InvoiceCompanyScopeNoTypeIndexName, StringComparison.Ordinal) ||
                ...
                foreach (var indexName in indexNames)
                {
                    ...
                    bool isLegacyInvoiceIndex =
                        (columns.Count == 1 && string.Equals(columns[0], "InvoiceNo", StringComparison.Ordinal)) ||
                        (columns.Count == 2 &&
                         string.Equals(columns[0], "InvoiceNo", StringComparison.Ordinal) &&
                         string.Equals(columns[1], "Type", StringComparison.Ordinal));
                    if (isLegacyInvoiceIndex)
                    {
            ...
            try
            {
                ...
                await using (var command = connection.CreateCommand())
                {
                    command.CommandText = """
                        ...
                        CROSS JOIN LATERAL (
                            SELECT ARRAY_AGG(attribute.attname::text ORDER BY indexed_column.ordinal_position) AS key_columns
                            ...
                            WHERE indexed_column.ordinal_position <= index_info.indnkeyatts
                        ) indexed_columns
                        WHERE table_class.relname = 'Invoices'
                          AND table_namespace.nspname = current_schema()
                          AND index_info.indisunique = true
                          AND (
                              indexed_columns.key_columns = ARRAY['InvoiceNo']::text[]
                              OR indexed_columns.key_columns = ARRAY['InvoiceNo', 'Type']::text[]
                          )
                        """;
                    await using var reader = await command.ExecuteReaderAsync().ConfigureAwait(false);
                    while (await reader.ReadAsync().ConfigureAwait(false))
                ...
                foreach (var indexName in indexNames.Where(item =>
                             ...
                             !string.Equals(item.IndexName, InvoiceCompanyScopeNoTypeIndexName, StringComparison.Ordinal)))
                {
