...
fn classify_installer_exit_code(code: Option<i32>) -> Result<InstallOutcome, String> {
    match code {
        ...
        Some(1618) => Err(
            "Windows 正在执行另一项安装。请等待其完成，再双击本程序继续安装 WebView2。".to_owned(),
        ),
        ...
        None => Err(
            "WebView2 Runtime 安装器被异常终止，未返回退出码。请重新运行本程序重试。".to_owned(),
        ),
    }
}
...
fn wait_for_installer(child: &mut Child, timeout: Duration) -> Result<ExitStatus, String> {
    ...
    loop {
        ...
        match result {
            ...
            _ => {
                crate::platform_process::terminate_child_tree(child);
...
mod tests {
    ...
    fn installer_exit_codes_distinguish_cancel_restart_and_failure() {
        ...
        assert!(
            classify_installer_exit_code(Some(1618))
                .unwrap_err()
                .contains("另一项安装")
        );
        assert!(
            classify_installer_exit_code(Some(1603))
                .unwrap_err()
                .contains("0x00000643")
        );
    ...
    fn timed_out_installer_is_stopped_before_returning() {
        ...
        let mut child = Command::new(executable)
            ...
            .unwrap();
        assert!(
            wait_for_installer(&mut child, Duration::ZERO)
                .unwrap_err()
                .contains("超时")
        );
