...
use crate::{desktop_commands, desktop_runtime, runtime_paths::RuntimePaths};
...
fn build_tauri_updater(
    ...
    builder = builder.on_before_exit(move || {
        desktop_commands::confirm_app_exit();
        if let Err(error) = desktop_runtime::stop(&app_for_exit) {
...
mod tests {
    ...
    fn runtime_endpoint_empty_value_uses_packaged_default() {
        ...
        assert!(
            parse_runtime_endpoint(Some("   ".to_owned()))
                .unwrap()
                .is_none()
        );
    }
