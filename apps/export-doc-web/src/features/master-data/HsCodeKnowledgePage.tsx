...
function KnowledgeSearch({ client, canOperate }: { client: ExportDocManagerApiClient; canOperate: boolean }) {
  ...
  const feedbackMutation = useMutation({
    ...
    onSuccess: (_, value) => { setFeedback({ tone: "success", message: value.accepted ? "已记录：当前商品与该候选编码匹配，用于优化后续推荐。" : "已记录：当前商品与该候选编码不匹配，用于优化后续推荐。" }); void search.refetch(); },
  ...
  return <div className="knowledge-task-card knowledge-search-card">
    ...
    {search.data ? <>
      ...
      {canOperate ? <div className="knowledge-feedback-explainer" role="note"><div><strong>推荐反馈</strong><span>下面的按钮只表示“当前商品与候选编码是否匹配”，不是 HS 编码本身同时处于适用和不适用状态。</span></div><small>不确定时可以不操作，不影响查询和查看。</small></div> : null}
      <div className="knowledge-result-list">{search.data.items.map(item => <article key={`${item.currentCode}-${item.rawCode}`} className={`knowledge-result ${item.canUse ? "usable" : "warning"}`}>
        <div>
          ...
          <div className="knowledge-score"><b>{item.score}</b><span>匹配分</span><small>{item.exampleCount} 条实例</small>{canOperate ? <div className="knowledge-feedback-actions" aria-label="对当前推荐结果反馈"><span>当前商品是否匹配？</span><button type="button" title="确认当前商品与此编码匹配" disabled={!item.canUse || feedbackMutation.isPending} onClick={() => feedbackMutation.mutate({ code: item.currentCode, accepted: true, name: item.name, specification: item.specification })}>确认匹配</button><button className="text-button" type="button" title="标记当前商品与此编码不匹配" disabled={feedbackMutation.isPending} onClick={() => feedbackMutation.mutate({ code: item.currentCode || item.rawCode, accepted: false, name: item.name, specification: item.specification })}>标记不匹配</button></div> : null}</div>
      </article>)}</div>
...
function RemoteCandidateReview({ client, canOperate, canBatchOperate }: { client: ExportDocManagerApiClient; canOperate: boolean; canBatchOperate: boolean }) {
  ...
  return <section className="remote-candidate-review"><div className="section-heading"><div><h2>联网候选审核</h2><p>待审核、已确认和已忽略记录分别保存；历史决定可以恢复为待审核。</p></div><button className="secondary-button" type="button" onClick={() => void candidates.refetch()}><RefreshCw size={16}/>刷新</button></div>
    ...
    <div className="knowledge-table">{candidates.data?.items.map(item => <HsRemoteCandidateCard key={item.id} client={client} candidate={item} status={status} canOperate={canOperate} allowSelection={canBatchOperate} selected={selected.has(item.id)} draft={drafts[item.id] ?? ""} isBusy={review.isPending || batchReview.isPending} onToggle={() => toggle(item.id)} onDraft={value => setDrafts(current => ({...current,[item.id]:value}))} onConfirm={code => review.mutate({id:item.id,code,confirmed:true})} onIgnore={() => review.mutate({id:item.id,code:"",confirmed:false})} onReset={() => reset.mutate([item.id])} />)}</div>
...
function KnowledgeTransfer({client,canImport,canExport}:{client:ExportDocManagerApiClient;canImport:boolean;canExport:boolean}) { const input=useRef<HTMLInputElement>(null);const[message,setMessage]=useState("");const importer=useMutation({mutationFn:(file:File)=>client.importHsCodeKnowledge({body:file}),onSuccess:value=>setMessage(value.result.message),onError:error=>setMessage(readApiError(error))});async function download(){const blob=await client.exportHsCodeKnowledge();downloadBlob(blob,`ExportDocManager-HsLibrary-${new Date().toISOString().slice(0,10).replace(/-/g,"")}.edmhs`);}return <div className="knowledge-task-grid"><article className="knowledge-task-card"><Download size={28}/><h2>导出本机知识库</h2><p>包含已验证税则、正式申报实例、替代关系和学习记录，不包含业务单据和账号数据。</p>{canExport?<button className="command-button" type="button" onClick={()=>void download()}>导出 .edmhs</button>:<span className="muted-text">需要管理权限</span>}</article><article className="knowledge-task-card"><Upload size={28}/><h2>导入另一台电脑</h2><p>导入前校验完整性；缺少有效编码来源、年度或验证时间的知识包会被拒绝。</p><input ref={input} hidden type="file" accept=".edmhs" onChange={event=>{const file=event.currentTarget.files?.[0];event.currentTarget.value="";if(file)importer.mutate(file);}}/>{canImport?<button className="command-button" type="button" disabled={importer.isPending} onClick={()=>input.current?.click()}>选择知识库文件</button>:<span className="muted-text">当前权限只能查看</span>}</article>{message?<InlineNotice tone={importer.isError?"error":"success"} className="knowledge-transfer-message">{message}</InlineNotice>:null}</div>; }
