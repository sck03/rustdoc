...
export function SupplierDirectoryPage({ businessDate, client }: { businessDate: string; client: ExportDocManagerApiClient }) {
  ...
  async function deleteSupplier() {
    if (!supplierPermission.canManage || !selectedSupplier || !await requestConfirmation({ title: "删除或停用供应商", description: `确定处理供应商“${selectedSupplier.name}”吗？`, details: ["没有供货关系和评价时，将彻底删除供应商及其联系人。", "存在供货关系或评价时，将保留联系人和历史资料，并把供应商状态改为“停用”。"], confirmLabel: "继续处理", tone: "danger" }) || !await confirmDiscardChanges("处理供应商")) return;
    try { const response = await client.deleteSupplier({ id: selectedSupplier.id }); setSupplierDraftDirty(false); setContactDraftDirty(false); await loadSupplierOptions(undefined, ""); setRevision((value) => value + 1); setView("directory"); setFeedback(successFeedback(response.message)); }
    catch (error) { setFeedback(requestErrorFeedback(error)); }
  }
  ...
  async function saveContact(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const rows = await client.listSupplierContacts({ supplierId }); setContacts(rows); setContactId(saved.id); setContactDraftDirty(false); setContactView("editor"); setFeedback(successFeedback(id ? "联系人已更新。" : "联系人已添加。"));
    } catch (error) { setFeedback(requestErrorFeedback(error)); }
  }

  async function deleteContact() {
    if (!supplierPermission.canManage || !selectedContact || !await requestConfirmation({ title: "删除供应商联系人", description: `确定删除联系人“${selectedContact.name}”吗？`, confirmLabel: "确认删除", tone: "danger" }) || !await confirmDiscardChanges("删除联系人")) return;
    try {
      ...
      setContacts(rows); setContactId(rows[0]?.id ?? 0); setContactDraftDirty(false); setContactView("directory"); setFeedback(successFeedback("联系人已删除。"));
    }
    catch (error) { setFeedback(requestErrorFeedback(error)); }
  }
  ...
  return <section className="work-surface">
    ...
    {view === "directory" ? <section className="form-section" {...getTaskViewPanelProps(supplierTabsId, "directory")}><div className="section-header"><div><h3>供应商目录</h3><p className="section-description">查找供货单位并进入资料、联系人或供应产品。</p></div><div className="section-header-actions"><span>共 {page?.totalCount ?? 0} 家</span>{supplierPermission.canOperate ? <button className="primary-button" type="button" onClick={() => void beginNewSupplier()}>新建供应商</button> : null}</div></div>
    ...
    {view === "profile" ? <form className="form-grid" key={newSupplier ? "new" : `${selectedSupplier?.id ?? "empty"}-${selectedSupplier?.versionNumber ?? 0}`} onSubmit={saveSupplier} {...getTaskViewPanelProps(supplierTabsId, "profile")}>
        <div className="section-heading-row"><h3>{newSupplier ? "新建供应商" : supplierPermission.canOperate ? "供应商资料" : "查看供应商"}</h3>{supplierPermission.canOperate ? <button className="secondary-button" type="button" onClick={() => void beginNewSupplier()}>新建</button> : null}</div>
        ...
        <fieldset className="permission-fieldset form-field-wide" disabled={!supplierPermission.canOperate} onChangeCapture={() => setSupplierDraftDirty(true)}>
    ...
    {view === "contacts" && selectedSupplier && !newSupplier ? <section className="form-section supplier-contact-workspace" {...getTaskViewPanelProps(supplierTabsId, "contacts")}>
      ...
      {contactView === "directory" ? <>
        <div className="section-header-actions supplier-contact-directory-actions"><button className="secondary-button" type="button" onClick={() => void changeView("profile")}>返回供应商资料</button>{supplierPermission.canOperate ? <button className="primary-button" type="button" onClick={() => void selectContact(0)}>新增联系人</button> : null}</div>
        ...
        <fieldset className="permission-fieldset form-field-wide" disabled={!supplierPermission.canOperate} onChangeCapture={() => setContactDraftDirty(true)}>
        ...
        <div className="form-actions">{supplierPermission.canOperate ? <button className="primary-button" type="submit" disabled={!supplierId}>保存联系人</button> : null}{selectedContact && supplierPermission.canManage ? <button className="secondary-button danger-button" type="button" onClick={() => void deleteContact()}>删除</button> : null}</div>
      </form>}
    </section> : null}
