...
export const workspaceNavGroups: WorkspaceNavGroupConfig[] = [
  ...
  },
  {
    ...
    items: [
      { label: "客户与跟进", description: "维护销售客户，记录沟通和下一步跟进", to: "/crm/follow-ups", icon: ContactRound,
        isActive: (path) => path.startsWith("/crm/follow-ups"), workspace: "sales", moduleKey: "sales.crm", permissionMatch: "any",
        requiredPermissions: [
          { resourceKey: permissionResources.crmCustomers, action: permissionActions.view },
  ...
  },
  {
    ...
    items: [
      ...
      { label: "单一窗口", description: "准备申报资料，处理交接批次与回执", to: "/single-window/operation-center", icon: Network,
        isActive: (path) => path.startsWith("/single-window"), children: [
      { label: "申报业务", description: "准备申报资料，处理交接批次与回执", to: "/single-window/operation-center", icon: Network,
  ...
  },
  {
    ...
    items: [
      ...
      { label: "Excel 模板与托单", description: "导出导入模板、空白托单和 Excel 转托单", keywords: "Excel 工具", to: "/tools/excel", icon: FileSpreadsheet,
  ...
  },
  {
    ...
    items: [
      { label: "系统设置", description: "设置运行环境、邮件和业务参数，备份与维护数据", to: "/settings", icon: Settings,
        ...
        searchItems: settingsFeatureLinks.map((item) => ({ ...item, icon: Settings, isActive: (path: string) => path.startsWith("/settings") })) },
      { label: "账号与权限", description: "管理登录账号、权限方案和数据范围", to: "/system/access-control", icon: UsersRound,
        isActive: isAccessControlRoute, requiresAdmin: true, requiresSystemAdministration: true, searchItems: [
          { label: "权限方案与模块权限", description: "系统管理 / 账号与权限 / 权限方案", keywords: "资源 动作 数据范围 授权", to: "/system/access-control?view=templates", search: "view=templates", icon: ShieldCheck, isActive: isAccessControlRoute },
        ] },
      ...
      { label: "关于与支持", description: "查看产品信息、授权注册和软件更新", to: "/system/about", icon: Info,
        isActive: (path) => path.startsWith("/system/about") || isLicenseRoute(path) || path.startsWith("/system/update"), showSectionNav: true, children: [
