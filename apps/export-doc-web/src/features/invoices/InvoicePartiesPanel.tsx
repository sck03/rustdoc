import { Building2, ContactRound, RefreshCw, type LucideIcon } from "lucide-react";
import type {
  ApiCustomerDto,
  ApiExporterDto,
  ApiInvoiceDetailDto,
...
import { SelectField, TextAreaField, TextField } from "../../ui/FormFields.tsx";
    ...
    <section className="form-section information-tier-required" aria-label="客户与出口商">
      ...
      <div className="invoice-party-groups">
        <section className="invoice-party-group" aria-label="客户信息">
          <PartyGroupHeading icon={ContactRound} title="客户信息" description="选择档案自动填入，修改仅用于本张发票"
            status={selectedCustomer ? "已关联档案" : "手工资料"} connected={Boolean(selectedCustomer)} />
          <div className="field-grid">
            <RemoteSelectField<ApiCustomerDto>
              className="field-grid-span-all" label="客户档案"
              value={invoice.customerId && invoice.customerId > 0 ? String(invoice.customerId) : ""}
              selectedOption={selectedCustomer} selectedLabel={invoice.customerNameEN || undefined}
              disabled={isBusy || !isEditable} queryKey={["master-data", "customers", "lookup"]}
              loadOptions={async (keyword, signal) => (await client.listCustomersPage({ keyword: keyword || undefined, pageNumber: 1, pageSize: 50 }, { signal })).items}
              getValue={(customer) => String(customer.id)}
              getLabel={(customer) => customer.displayName || customer.customerNameEN || "-"} onChange={applyCustomer}
            />
            <TextField className="field-grid-span-all" label="客户英文名" value={invoice.customerNameEN} disabled={!isEditable} onChange={(value) => onChange({ customerNameEN: value })} />
          </div>
          <details className="invoice-inline-details invoice-party-details">
            <summary>地址与通知人 <small>{invoice.notifyPartyMode === "SameAsConsignee" ? "同收货人" : invoice.notifyPartyMode === "Separate" ? "独立通知人" : "无通知人"}</small></summary>
            <div className="field-grid invoice-inline-details-grid">
              <TextAreaField className="field-grid-span-all invoice-party-address-field" label="客户英文地址" value={invoice.customerAddressEN ?? ""} disabled={!isEditable} onChange={(value) => onChange({ customerAddressEN: value })} />
              <SelectField className="field-grid-span-all" label="通知人方式" value={invoice.notifyPartyMode} disabled={!isEditable} includeEmptyOption={false}
                options={[{ value: "None", label: "无通知人" }, { value: "SameAsConsignee", label: "同收货人" }, { value: "Separate", label: "独立通知人" }]}
                onChange={(value) => onChange({ notifyPartyMode: value as ApiInvoiceDetailDto["notifyPartyMode"], ...(value === "Separate" ? {} : { notifyPartyName: "", notifyPartyAddress: "" }) })} />
              {invoice.notifyPartyMode === "Separate" && <>
                ...
                <TextAreaField className="field-grid-span-all invoice-party-address-field" label="通知人英文地址" value={invoice.notifyPartyAddress ?? ""} disabled={!isEditable} onChange={(value) => onChange({ notifyPartyAddress: value })} />
              </>}
            </div>
          </details>
        </section>
        <section className="invoice-party-group invoice-party-group-exporter" aria-label="出口商与收款信息">
          <PartyGroupHeading icon={Building2} title="出口商信息" description="常用企业资料自动填入，银行和印章按需查看"
            status={selectedExporter ? "已关联档案" : "手工资料"} connected={Boolean(selectedExporter)} />
          <div className="field-grid">
            <RemoteSelectField<ApiExporterDto>
              className="field-grid-span-all" label="出口商档案"
              value={invoice.exporterId && invoice.exporterId > 0 ? String(invoice.exporterId) : ""}
              selectedOption={selectedExporter} selectedLabel={invoice.exporterNameEN || invoice.exporterNameCN || undefined}
              disabled={isBusy || !isEditable} queryKey={["master-data", "exporters", "lookup"]}
              loadOptions={async (keyword, signal) => (await client.listExportersPage({ keyword: keyword || undefined, pageNumber: 1, pageSize: 50 }, { signal })).items}
              getValue={(exporter) => String(exporter.id)} getLabel={(exporter) => exporter.exporterNameEN || exporter.exporterNameCN || "-"} onChange={applyExporter}
            />
            <TextField className="field-grid-span-all" label="出口商英文名" value={invoice.exporterNameEN} disabled={!isEditable} onChange={(value) => onChange({ exporterNameEN: value })} />
            <TextField className="field-grid-span-all" label="出口商中文名" value={invoice.exporterNameCN ?? ""} disabled={!isEditable} onChange={(value) => onChange({ exporterNameCN: value })} />
          </div>
          <details className="invoice-inline-details invoice-party-details">
            <summary>地址与海关资料</summary>
            <div className="field-grid invoice-inline-details-grid">
              <TextAreaField className="field-grid-span-all invoice-party-address-field" label="出口商英文地址" value={invoice.exporterAddressEN ?? ""} disabled={!isEditable} onChange={(value) => onChange({ exporterAddressEN: value })} />
              <TextAreaField className="field-grid-span-all invoice-party-address-field" label="出口商中文地址" value={invoice.exporterAddressCN ?? ""} disabled={!isEditable} onChange={(value) => onChange({ exporterAddressCN: value })} />
