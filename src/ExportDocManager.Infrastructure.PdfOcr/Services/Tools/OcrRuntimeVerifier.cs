...
{
    public sealed class OcrRuntimeVerifier : IOcrRuntimeVerifier
    {
        internal static readonly TimeSpan RuntimeVerificationTimeout = TimeSpan.FromMinutes(3);
        ...
        public OcrRuntimeVerifier(IAppPathProvider pathProvider)
        {
            _pathProvider = pathProvider ?? throw new ArgumentNullException(nameof(pathProvider));
        }

        public async Task<OcrRuntimeVerificationResult> VerifyAsync(
            CancellationToken cancellationToken = default)
        {
            await using var host = new RustOcrSidecarHost(_pathProvider);
            if (!host.IsAvailable(out _))
            {
                throw new InfrastructureServiceException("未找到 Rust PP-OCRv6 Sidecar，无法执行跨平台 OCR 发布验证。");
            }

            string jobRoot = Path.Combine(_pathProvider.CacheRoot, "OcrJobs", $"verify-{Guid.NewGuid():N}");
            ...
            try
            {
                await File.WriteAllBytesAsync(
                    ...
                    cancellationToken);
                OcrResult result = await host.RecognizeAsync(
                    imagePath,
