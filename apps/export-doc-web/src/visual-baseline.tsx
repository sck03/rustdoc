...
function BaselineApp() {
  ...
  return <HashRouter><WorkspaceShell pathname={pathnameByPage[page] ?? "/dashboard"} apiBaseUrl="http://127.0.0.1:5188" isDesktopRuntime={page === "state-offline-local"} user={user} onLogout={() => undefined} connectivityOverride={page === "state-offline" || page === "state-offline-local" ? "offline" : "online"} notice={page === "state-route-redirect" ? { id: "permission", tone: "warning", title: "当前页面不可用", message: "当前权限模板未启用报表设计，系统已返回当前账号可以使用的工作区。" } : null} onDismissNotice={() => undefined}>
...
function ReportBaseline() { return <section className="work-surface report-template-surface"><div className="report-template-sticky-header"><div className="editor-toolbar"><div className="editor-title">报表设计</div><div className="toolbar-actions"><button type="button" className="command-button secondary">刷新</button><button type="button" className="command-button secondary">预览</button><button type="button" className="command-button">保存</button></div></div><div className="report-template-workspace-tabs"><button type="button" className="segmented-active">设计</button><button type="button">预览</button></div></div><div className="report-template-grid report-template-grid-design report-template-grid-new"><aside className="report-template-sidebar"><div className="template-selection-panel"><label>类型<select><option>出口单证</option></select></label><label>默认模板<select><option>invoice_template</option></select></label><label>我的 / 共享模板<select><option>未选择</option></select></label></div><details className="template-management-panel template-actions-panel template-user-panel" open><summary><span>我的模板</span><small>默认私有，可明确共享</small></summary><div className="template-management-content"><button type="button" className="secondary-button">复制当前模板</button></div></details><details className="template-management-panel template-actions-panel template-admin-panel"><summary><span>模板操作</span><small>invoice_template.html</small></summary></details><details className="template-management-panel template-package-panel"><summary><span>模板包</span><small>导入 / 导出</small></summary></details></aside><main className="report-template-new-designer"><div className="form-section" style={{minHeight:420}}><h2>模板画布</h2><div className="table-frame"><table><tbody><tr><td>EXPORTER</td><td>INVOICE NO.</td></tr><tr><td>CONSIGNEE</td><td>DATE</td></tr></tbody></table></div></div></main></div></section>; }
