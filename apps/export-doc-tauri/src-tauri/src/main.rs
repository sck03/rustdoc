...
fn main() {
    ...
    if args.iter().any(|arg| arg == "--ai-review-worker") {
        if let Err(error) = export_doc_engine::ai_worker() {
            eprintln!("{error}");
            std::process::exit(1);
        }
        return;
...
fn run_tauri_app() -> tauri::Result<()> {
    ...
    let app = builder
        .invoke_handler(tauri::generate_handler![
            ...
            desktop_file_dialog_commands::select_report_template_file,
            ...
            desktop_file_dialog_commands::select_save_report_template_file_path,
            ...
            desktop_runtime::get_desktop_runtime_context
        ])
        ...
        .on_window_event(|window, event| {
            ...
            if let tauri::WindowEvent::CloseRequested { api, .. } = event {
                ...
                api.prevent_close();
                if desktop_runtime::begin_graceful_shutdown(window.app_handle()) {
                    let _ = window.hide();
                }
            }
        })
    ...
    app.run(|app_handle, event| {
        if let tauri::RunEvent::ExitRequested { api, .. } = event {
            ...
            if desktop_runtime::begin_graceful_shutdown(app_handle) {
                api.prevent_exit();
                if let Some(window) = app_handle.get_webview_window("main") {
                    let _ = window.hide();
                }
...
mod tests {
    ...
    fn diagnostic_log_writer_reports_failure_when_no_root_is_writable() {
        ...
        assert!(
            append_diagnostic_log_to_roots(
                &[first, second],
                "tauri-errors.log",
                "startup failed",
                "permission denied",
