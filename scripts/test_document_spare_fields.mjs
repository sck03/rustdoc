...
const paymentSchema=api.parseReportDesignerV3Source("", "PaymentVoucher").schema;
paymentSchema.layers.find(layer=>layer.role === "Body").elements=["Payment.CNYAmount","Payment.AccountNo","Payment.Spare10"].map((field,index)=>api.createV3FieldElement(field,1000,1000+index*600));
assert.match(api.renderReportDesignerLocalPreviewSample(JSON.stringify(paymentSchema), "paymentVoucher"), /12345\.67[\s\S]*6222[\s\S]*备用 10 示例/);
assert.equal(api.renderReportDesignerLocalPreviewSample("<html>old</html>", "exportStandard"), "", "HTML template sources are rejected");
