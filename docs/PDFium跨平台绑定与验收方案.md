...
> 2026-09-20 实施更新:源码已开始使用 `pdfium-render = 0.9.4`(关闭默认 feature,显式启用 `pdfium_7881` 和 `image_025`)替换 engine 内的手写 PDFium 绑定;原生包版本仍为 `152.0.7961`,`155.0.8057` 升级和跨平台运行验收尚未实施。当前未提交 PDFium 二进制到仓库,本机只完成了 engine 编译验证,真实预览/文字/OCR/合并和 Windows x64 运行回环仍需在带受管原生资源的构建环境中执行。

> 历史审查：以下第 1—8 节保留 2026-09-20 迁移前评估；其中“当前手写绑定”均指当时提交，不能覆盖上面的实施更新。2026-09-21 已确认源码为 pdfium-render 封装。原审查对照当时 Rust 源码、PDFium `chromium/7961` / `chromium/8057` 公共头文件、`pdfium-render 0.9.4` 发布归档及本地 Windows/Linux x64 原生资源。本文修正绑定选型与 ABI 说明，不表示已替换依赖或完成目标平台运行验收。
