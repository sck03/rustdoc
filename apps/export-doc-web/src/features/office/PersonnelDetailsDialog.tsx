...
import type { ApiUserDto, ExportDocManagerApiClient, PersonnelDepartmentRecord, PersonnelRecord } from "../../api/index.ts";
    ...
    <OfficeDialog title={record ? `${record.employee.fullName} · 人员档案` : "人员档案"} onClose={onClose}>
      {query.isPending ? <PageState tone="loading" title="正在读取人员档案" /> : query.isError ? <PageState tone="error" title="档案加载失败" description={readApiError(query.error)}
        ...
        <div className="office-card-actions">
          {record.canEdit && <button type="button" className="command-button" onClick={() => setEditing(record)}>维护档案</button>}
          {personnelWorkflows(record).map((action) => <button type="button" className="command-button secondary" key={action} onClick={() => setWorkflow({ record, action })}>{personnelActionLabels[action]}</button>)}
          {record.canLinkAccount && <button type="button" className="command-button secondary" onClick={() => setLinking(record)}>关联账号</button>}
        </div>
        <nav className="office-tabs" aria-label="人员档案内容">{[["profile", "档案信息"], ["history", "任职与操作记录"], ["clearance", "交接事项"]].map(([key, label]) =>
          <button type="button" key={key} aria-pressed={tab === key} onClick={() => setTab(key)}>{label}</button>)}</nav>
...
function PersonnelFacts({ record, user }: { record: PersonnelRecord; user: ApiUserDto }) {
  ...
  return <div className="personnel-detail-stack">
    ...
    <dl className="personnel-facts">{facts.map(([label, value]) => <div key={label} className={["工作邮箱", "工作地点", "关联账号"].includes(label ?? "") ? "personnel-wide" : undefined}><dt>{label}</dt><dd>{value || "未登记"}</dd></div>)}
