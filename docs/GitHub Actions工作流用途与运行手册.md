...
Docker 本地验收:运行 `scripts/run-native-docker.cmd`,默认只绑定 `127.0.0.1:5188`;首次管理员初始化使用 `deploy/rust-native/runtime/bootstrap-token.txt`。远端生命周期由 `rust-native-container-release.yml` 执行,不把普通 PR 当作容器发布。
