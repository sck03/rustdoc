...
namespace ExportDocManager.Services.Data
{
    public partial class ExcelImportService
    {
        ...
        {
            return FindValueBesideLabel(worksheet, labels, allowCombinedLabel: false, sameRowOnly: false);
        }

        private string FindValueBesideLabel(
            IExcelImportWorksheet worksheet,
            IReadOnlyList<string> labels,
            bool allowCombinedLabel,
            bool sameRowOnly)
        {
            for (int row = 1; row <= 80; row++)
            {
                for (int column = 1; column <= 30; column++)
                {
                    ...
                    bool isMatchingLabel = allowCombinedLabel
                        ? ContainsAnyLabel(value, labels)
                        : IsAnyLabel(value, labels);
                    if (string.IsNullOrWhiteSpace(value) || !isMatchingLabel)
                    {
                        continue;
                    }
        ...
        private static bool ContainsAnyLabel(string value, IReadOnlyList<string> labels)
        {
            string normalizedValue = NormalizeLabel(value);
            return !string.IsNullOrWhiteSpace(normalizedValue)
                && labels.Select(NormalizeLabel).Any(label =>
                    !string.IsNullOrWhiteSpace(label)
                    && (normalizedValue == label || normalizedValue.Contains(label, StringComparison.Ordinal)));
        }
