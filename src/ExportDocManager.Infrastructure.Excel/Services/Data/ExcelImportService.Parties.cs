...
namespace ExportDocManager.Services.Data
{
    public partial class ExcelImportService
    {
            ...
            ExcelImportAnalysisReport analysisReport)
        {
            try
            {
                var customerPartyBlock = GetUsablePartyBlock(GetAnalyzedValue(analysisReport, "CustomerNameEN"))
                    ...
                    ?? GetUsablePartyBlock(GetValueByLabelsOrCell(
                        worksheet,
                        settings.CustomerNameCell,
                        ["收货人", "consignee name", "customer"],
                        configuredValueValidator: ExcelImportPartyTextClassifier.IsPlausiblePartyName));
                if (customerPartyBlock == null)
                {
                    customerPartyBlock = GetUsablePartyBlock(GetValueByLabelsOrCell(
                        worksheet,
                        settings.CustomerNameCell,
                        ["consignee"],
                        configuredValueValidator: ExcelImportPartyTextClassifier.IsPlausiblePartyName));
                ...
                var analyzedNotifyPartyBlock = GetUsablePartyBlock(GetAnalyzedValue(
                        analysisReport,
                        "NotifyPartyName",
                        IsUsableNotifyPartyValue));
                string explicitNotifyPartyValue = FindValueBesideLabel(
                    worksheet,
                    ["通知人", "通知方", "notify party name", "notify party"],
                    allowCombinedLabel: true,
                    sameRowOnly: true);
                var sheetNotifyPartyBlock = IsSameAsConsignee(explicitNotifyPartyValue)
                    ? (NormalizeExcelTextBlock(explicitNotifyPartyValue), string.Empty)
                    : GetUsablePartyBlock(explicitNotifyPartyValue)
                    ?? FindPartyBlockBelowLabel(worksheet, ["通知人", "通知方", "notify party name", "notify party"]);
                var notifyPartyBlock = IsSameAsConsignee(sheetNotifyPartyBlock?.Name ?? string.Empty)
                    ? sheetNotifyPartyBlock
                    : analyzedNotifyPartyBlock
                        ?? sheetNotifyPartyBlock
                    ?? GetUsablePartyBlock(GetValueByLabelsOrCell(
                        worksheet,
                        settings.NotifyPartyNameCell,
                        ["通知人", "通知方", "notify party name"],
                        configuredValueValidator: IsUsableNotifyPartyValue));
                if (notifyPartyBlock == null)
                {
                    notifyPartyBlock = GetUsablePartyBlock(GetValueByLabelsOrCell(
                        worksheet,
                        settings.NotifyPartyNameCell,
                        ["notify party"],
                        configuredValueValidator: IsUsableNotifyPartyValue));
                ...
                if (customerPartyBlock != null)
                {
                    ...
                    bool notifyPartyMatchesConsignee = IsSameAsConsignee(notifypartyName)
                        || IsSameAsConsignee(notifyPartyAddressFromBlock);
                    ...
                    string notifypartyAddress = string.Empty;
                    if (!notifyPartyMatchesConsignee)
                    {
                        notifypartyAddress = GetAnalyzedValue(analysisReport, "NotifyPartyAddress", IsUsableNotifyPartyValue)
                            ?? GetValueByLabelsOrCell(
                                worksheet,
                                settings.NotifyPartyAddressStartCell,
                                ["notify party", "通知人地址", "通知方地址"],
                                configuredValueValidator: IsUsableNotifyPartyValue);
                    ...
                    if (notifyPartyMatchesConsignee || IsSameAsConsignee(notifypartyAddress))
                    {
                        invoice.NotifyPartyMode = NotifyPartyMode.SameAsConsignee;
                        customer.NotifyPartyMode = NotifyPartyMode.SameAsConsignee;
                        notifypartyName = string.Empty;
                        notifypartyAddress = string.Empty;
                    }
                    else if (!string.IsNullOrWhiteSpace(notifypartyName) ||
                             !string.IsNullOrWhiteSpace(notifypartyAddress))
                    {
                        invoice.NotifyPartyMode = NotifyPartyMode.Separate;
                        customer.NotifyPartyMode = NotifyPartyMode.Separate;
                    }
                    else
                    {
                        invoice.NotifyPartyMode = NotifyPartyMode.None;
                        customer.NotifyPartyMode = NotifyPartyMode.None;
                    }
        ...
        private static bool IsUsableNotifyPartyValue(string value)
        {
            return IsSameAsConsignee(value)
                || ExcelImportPartyTextClassifier.IsPlausiblePartyName(value)
                || IsUsablePartyAddressValue(value);
        }
