// Docker build component: shared, verified native OCR and PDF library extraction.
...
const manifest = JSON.parse(await readFile(path.join(root, "eng/native-runtime-packages.json"), "utf8").then(text=>text.replace(/^\uFEFF/u,"")));
await prepare("microsoft.ml.onnxruntime", "libonnxruntime.so", path.join(destination, "ocr"), ["LICENSE", "ThirdPartyNotices.txt"]);
await prepare("bblanchon.pdfium.linux", "libpdfium.so", path.join(destination, "pdf"), []);

async function prepare(packageName, libraryName, destination, noticeFiles) {
  await managed(destination);
  const { version, sha512: contentHash } = manifest.packages[packageName];
  if (manifest.schemaVersion!==1 || !/^\d+\.\d+\.\d+$/.test(version) || Buffer.from(contentHash,"base64").length!==64) throw new Error("Invalid governed native package manifest.");
  const cache = path.join(root, ".codex-runtime", "native-runtime-packages");
  await managed(cache);
  await mkdir(cache, { recursive: true });
  const archive = path.join(cache, `${packageName}.${version}.nupkg`);
  if (!await matches(archive, contentHash)) {
    const temporary = `${archive}.${randomUUID()}.download`;
    const controller = new AbortController();
    const timer = setTimeout(() => controller.abort(), 600_000);
    try {
      const response = await fetch(`https://api.nuget.org/v3-flatcontainer/${packageName}/${version}/${packageName}.${version}.nupkg`, { signal: controller.signal, redirect: "error" });
      ...
      const limit = new Transform({ transform(bytes, _encoding, callback) {
        size += bytes.length;
        callback(size > 2 * 1024 ** 3 ? new Error("Native package exceeds 2 GiB.") : null, bytes);
      } });
      await pipeline(Readable.fromWeb(response.body), limit, createWriteStream(temporary, { flags: "wx" }), { signal: controller.signal });
      if (!await matches(temporary, contentHash)) throw new Error("Native package does not match the governed archive SHA-512.");
      await rename(temporary, archive);
    } finally { clearTimeout(timer); await rm(temporary, { force: true }); }
  }
  await mkdir(destination, { recursive: true });
  const child = spawnProcessTree("unzip", ["-j", "-o", archive, `runtimes/${rid}/native/${libraryName}`, ...noticeFiles, "-d", destination], { cwd: root, stdio: "inherit", windowsHide: true });
  let extractionTimer;
  try {
  ...
  const library = path.join(destination, libraryName);
  await managed(library);
  const info = await stat(library);
  if (!info.isFile() || info.size <= 0 || info.size > 256 * 1024 ** 2) throw new Error("Extracted native library is invalid.");
  process.stdout.write(`Governed ${packageName} ${version} (${rid}) prepared.\n`);
