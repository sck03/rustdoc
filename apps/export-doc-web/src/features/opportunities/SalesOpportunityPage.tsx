...
export function SalesOpportunityPage({ client }: { client: ExportDocManagerApiClient }) {
  ...
  async function remove() {
    if (!opportunityPermission.canManage || !selected || !window.confirm(`删除商机“${selected.title}”？`)) return;
  ...
  return <section className="work-surface">
    ...
    {!opportunityPermission.canOperate ? <div className="permission-readonly-notice">当前权限模板仅允许查看商机；新建、修改和阶段更新已禁用。</div> : null}
    ...
    {view === "directory" ? <section className="form-section"><div className="section-header"><div><h3>商机目录</h3><p className="section-description">按客户、阶段和报价编号查找销售机会。</p></div><div className="section-header-actions"><span>共 {page?.totalCount ?? 0} 项</span>{opportunityPermission.canOperate ? <button className="primary-button" type="button" onClick={() => { setSelected(null); changeView("editor"); }}>新建商机</button> : null}</div></div>
      ...
      <div className="table-frame"><table className="data-table responsive-data-table"><thead><tr><th>商机</th><th>客户</th><th data-table-priority="secondary">产品</th><th>金额</th><th data-table-priority="secondary">概率</th><th>阶段</th><th /></tr></thead><tbody>
    ...
    {view === "editor" ? <form className="form-grid" key={selected?.id ?? "new"} onSubmit={save}>
      ...
      {!customers.length ? <div className="empty-guidance form-field-wide"><strong>先建立一位销售客户</strong><span>商机必须归属 CRM 客户，不会写入原单证客户资料。</span>{opportunityPermission.canOperate ? <button className="primary-button" type="button" onClick={() => navigate("/crm/follow-ups?view=profile")}>建立客户资料</button> : null}</div> : null}
    ...
    {selected && view === "history" ? <section className="form-section"><div className="section-header"><h3>阶段与报价版本历史</h3><span>{history.length} 个版本</span></div>
      <div className="table-frame"><table className="data-table"><thead><tr><th>版本</th><th>类型</th><th>阶段</th><th>报价编号</th><th>金额</th><th>概率</th><th>变更说明</th><th>操作信息</th></tr></thead><tbody>
