...
function BaselineApp() {
  ...
  return <HashRouter><WorkspaceShell pathname={pathnameByPage[page] ?? "/dashboard"} apiBaseUrl="http://127.0.0.1:5188" isDesktopRuntime={false} user={user} onLogout={() => undefined}>
...
function StateBaseline({ tone }: { tone: "loading" | "empty" | "error" | "permission" }) {
  ...
  return <section className="work-surface"><div className="section-header"><div><h2>统一页面状态</h2><p>加载、空数据、失败和权限状态使用一致的视觉与辅助语义。</p></div></div><PageState tone={tone} title={content[0]} description={content[1]} action={tone === "error" ? <button className="command-button">重新加载</button> : undefined}/>{tone === "permission" ? <PermissionNotice>如需操作权限，请联系系统管理员调整当前岗位模板。</PermissionNotice> : null}{tone === "empty" ? <FormGuidance title="先建立产品资料" description="供货关系必须关联现有产品，完成后即可返回继续。" action={<button className="secondary-button">打开产品资料</button>} /> : null}</section>;
}
...
function InvoiceBaseline() {
  ...
  return <section className="editor-surface"><div className="editor-toolbar"><div className="editor-title"><span>新建发票</span><span className="editor-save-state" data-state="dirty">有未保存修改</span></div></div><form className="invoice-form"><nav className="invoice-editor-section-nav"><button className="invoice-section-nav-item">发票表头</button><button className="invoice-section-nav-item invoice-section-nav-primary">商品明细</button><button className="invoice-section-nav-item">利润/信用证</button><button className="invoice-section-nav-item">预览导出</button></nav><div className="invoice-editor-sticky-actions"><div><strong>YH2026-024</strong><span>当前有未保存修改</span></div><button className="command-button">保存发票</button></div><section className="form-section"><div className="section-header"><h2>基本信息</h2></div><div className="form-grid">{["发票号","发票日期","客户","出口商","贸易条款","目的国"].map(x=><label key={x}>{x}<input value={x === "发票号" ? "YH2026-024" : ""} readOnly/></label>)}</div></section><section className="form-section"><div className="section-header"><div><h2>商品明细</h2><p className="section-description">视觉基准展示真实编辑表的主要字段；其余单位、体积和备用字段可通过列设置显示。</p></div></div><div className="table-frame item-editor-frame" tabIndex={0} aria-label="完整发票商品明细编辑表"><table className="item-editor-table" style={{minWidth: 3150}}><thead><tr>{columns.map(column=><th key={column}>{column}</th>)}</tr></thead><tbody><tr>{values.map((value,index)=><td key={columns[index]}>{index === 0 ? <button type="button" className="secondary-button">行操作</button> : <input aria-label={`第1行${columns[index]}`} value={value} readOnly/>}</td>)}</tr></tbody></table></div><div className="item-summary-bar"><span>1 行</span><span>数量 1,000</span><span>箱数 20</span><span>毛重 250.00</span><span>金额 USD 4,500.00</span></div></section></form></section>;
}
function InvoicePartiesBaseline() { return <section className="editor-surface"><div className="editor-toolbar"><div className="editor-title">新建发票</div></div><section className="form-section"><div className="section-header"><h2>客户与出口商</h2></div><div className="invoice-party-groups"><section className="invoice-party-group"><div className="invoice-party-group-heading"><strong>客户信息</strong><span>选择客户档案后可继续调整本张发票内容</span></div><div className="field-grid"><label>客户档案<select><option>未选择</option></select></label><label className="field-grid-span-2">客户英文名<input readOnly /></label><label className="field-grid-span-all">客户地址<input readOnly /></label></div></section><section className="invoice-party-group"><div className="invoice-party-group-heading"><strong>通知人信息</strong><span>与客户不同的收货通知对象可单独填写</span></div><div className="field-grid"><label className="field-grid-span-all">通知人<input value="NOTIFY PARTY CO., LTD." readOnly /></label><label className="field-grid-span-all">通知人地址<input value="ROOM 1808, COMMERCIAL BUILDING, HONG KONG" readOnly /></label></div></section><section className="invoice-party-group invoice-party-group-exporter"><div className="invoice-party-group-heading"><strong>出口商与收款信息</strong><span>企业身份、地址、海关信息和银行资料集中维护</span></div><div className="field-grid"><label>出口商档案<select><option>未选择</option></select></label><label className="field-grid-span-2">出口商英文名<input readOnly /></label><label>统一信用代码<input readOnly /></label></div></section></div></section></section>; }
function HsBaseline() { return <section className="work-surface hs-knowledge-surface"><div className="knowledge-workflow">{["联网获取","匹配税则","人工审核","实例入库","智能使用"].map((x,i)=><div className={i<3?"active":""} key={x}><span>{i+1}</span><strong>{x}</strong><small>流程说明</small></div>)}</div><nav className="hs-knowledge-nav">{["智能查询","申报实例库","历史资料学习","年度税则","换机迁移","联网补充"].map(x=><a className={x==="联网补充"?"active":""} key={x}>{x}</a>)}</nav><div className="knowledge-task-card"><h2>联网候选审核</h2><p className="knowledge-task-lead">网页实例用于提供申报经验；必须匹配当前年度税则并由人工确认后才会入库。</p><div className="knowledge-table">{[1,2,3].map(i=><article className="remote-candidate-card" key={i}><div className="remote-candidate-evidence"><div className="remote-candidate-title"><strong>男式棉制针织T恤衫</strong><span className="status-pill">网页推荐待核验</span></div><div className="remote-code-comparison"><span><small>网页实例编码</small><b>61091000</b></span><i>→</i><span className="current-code"><small>待确认当前编码</small><b>6109100000</b></span></div><p>棉制、针织、男式、短袖</p><div className="remote-candidate-meta"><small>查询词：男T恤</small><small>网页出现 6 次</small><small>来源：i5a6</small></div></div><div className="remote-candidate-actions"><button className="command-button">确认加入当前编码</button><button className="text-button">忽略此实例</button></div></article>)}</div></div></section>; }
function SingleWindowBaseline() { return <section className="editor-surface"><div className="editor-toolbar single-window-document-toolbar"><div className="editor-title">海关原产地证</div><div className="single-window-command-band"><div className="single-window-view-mode"><button className="active">标准模式</button><button>高级模式</button></div><div className="single-window-tool-group"><span className="single-window-tool-heading">草稿</span><button className="command-button secondary">回填空白</button></div><div className="single-window-tool-group"><button className="command-button secondary">预检</button></div><button className="command-button">保存草稿</button></div></div><section className="form-section"><div className="section-header"><h2>草稿状态</h2></div><div className="coo-completion-overview"><div className="coo-completion-heading"><div><span>录入完成度</span><strong>67%</strong></div><small>4/6 个关键项目已具备</small></div><div className="coo-completion-track"><span style={{width:"67%"}}/></div><div className="coo-completion-steps">{[["证书基础","已具备",true],["申报对象","已具备",true],["运输贸易","待补充",false],["商品明细","2 行",true],["附件","1 条",true],["预警","2 条",false]].map(([label,detail,complete])=><span className={complete?"complete":"pending"} key={String(label)}>{label}<b>{detail}</b></span>)}</div></div></section><section className="form-section"><div className="section-header"><h2>证书基本信息</h2></div><div className="form-grid">{["发票号","申请人","收货人","运输方式","签证机构","目的国"].map(x=><label key={x}>{x}<input readOnly/></label>)}</div></section></section>; }
function ReportBaseline() { return <section className="work-surface report-template-surface"><div className="report-template-sticky-header"><div className="editor-toolbar"><div className="editor-title">报表设计</div><div className="toolbar-actions"><button className="command-button secondary">刷新</button><button className="command-button secondary">预览</button><button className="command-button">保存</button></div></div><div className="report-template-workspace-tabs"><button className="segmented-active">设计</button><button>预览</button></div></div><div className="report-template-grid report-template-grid-new"><aside className="report-template-sidebar"><div className="template-selection-panel"><label>类型<select><option>出口单证</option></select></label><label>默认模板<select><option>商业发票</option></select></label><label>我的 / 共享模板<select><option>未选择</option></select></label></div><details className="template-management-panel template-actions-panel template-user-panel" open><summary><span>我的模板</span><small>默认私有</small></summary><div className="template-management-content"><button className="secondary-button">复制当前模板</button></div></details><details className="template-management-panel template-actions-panel template-admin-panel"><summary><span>模板操作</span><small>商业发票</small></summary></details><details className="template-management-panel template-package-panel"><summary><span>模板包</span><small>导入 / 导出</small></summary></details></aside><main className="report-template-new-designer"><div className="form-section" style={{minHeight:420}}><h2>模板画布</h2><div className="table-frame"><table><tbody><tr><td>EXPORTER</td><td>INVOICE NO.</td></tr><tr><td>CONSIGNEE</td><td>DATE</td></tr></tbody></table></div></div></main></div></section>; }
